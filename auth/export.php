<?php 
    ...
    if (isset($_GET['exp_with'])) {
        ...
        $query = "SELECT * FROM giltmarket_sales INNER JOIN giltmarket_admin ON giltmarket_admin.admin_id = giltmarket_sales.sale_by WHERE giltmarket_sales.sale_status = 0 ";
        if ($exp_with == 'month') {
        ...
        if ($exp_status != 'all') {
            $query .= " AND giltmarket_sales.sale_type = '" . $exp_status . "'";
        }
        ...
        if ($count_row > 0) {
            ...
            $fileName = "Giltmarket-Trades-" . $exp_status . "-sheet";
